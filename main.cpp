...
const int n=251; //number of nodes
...
int new_cell_lattice[n][n]; // temporal model
int new_dipole_direction[n][n];
double new_cell_deformation[n][n];

int nodes_elem[(n-1)*(n-1)][4]; //nodes in each element

int cell_migration [n][n];

// vessels --> can I inizialize the arrays as variable size?
int vessels_lattice_x [(n-1)*20][n-1]; // VISUAL: for each vessel, each line contains the x coordinates of nodes composing the vessel
int vessels_lattice_y [(n-1)*20][n-1]; // VISUAL: for each vessel, each line contains the x coordinates of nodes composing the vessel
int tip_x [(n-1)*20];
int tip_y [(n-1)*20];
...
int main()
{
    ...
    for (iteration=0;iteration<505;iteration++) //504 = 7 DAYS
    {
		...
		*****************************************************************************************/ 
		if (iteration==0)
	    {
		    ...
		    Initialize_model(cell_lattice,vessels_lattice_x, vessels_lattice_y, dipole_direction, nodes_elem, fibro_cell_number, endo_cell_number, number_vessels, tip_x, tip_y, previous_growth_dir, vessel_length, Nodes_x, Nodes_y);
	    ...
	    *****************************************************************************************/ 
	    ...
	    Read_displacements(cell_lattice, dipole_direction, cell_deformation, iteration, model, number_vessels, tip_x, tip_y, Nodes_x, Nodes_y);           
	
	    /**************************************************************************************
	                                        Create temporary FEM
	    *****************************************************************************************/
	    cout<<"CREATE TEMPORARY FEM"<<endl;
	    Create_temporal_FEM(cell_lattice, cell_deformation, new_cell_lattice, new_dipole_direction, new_cell_deformation, iteration, cell_migration, number_vessels, tip_x, tip_y, Nodes_x, Nodes_y, previous_growth_dir); 
	    ...
	    *****************************************************************************************/                 
	    ...
	    model=2;  //temporal model
	    Read_displacements(new_cell_lattice, new_dipole_direction, new_cell_deformation, iteration, model, number_vessels, tip_x, tip_y, Nodes_x, Nodes_y);           
	      
		/**************************************************************************************
                                                 Vessel growth
        *****************************************************************************************/ 
    	if ((iteration)%2==0) //every 2 iteration steps in order to have a growth of 15 micron per hour in avrage (if I use data of 15-20 micron per hour)
    	{
...
//    		Write_results_blood_vessel(cell_lattice,iteration);
	    ...
	    *****************************************************************************************/ 
	    ...
	    Update_model(cell_lattice, dipole_direction, cell_deformation, new_cell_lattice, new_dipole_direction, new_cell_deformation, iteration, cell_migration);
